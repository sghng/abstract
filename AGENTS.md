...
This file is the team's shared kernel: it is loaded into every agent session
and it survives compaction. It contains **only invariants** — the rules that
must never be forgotten. Procedures and templates live in skills; project
...
co-designed through consultation, executed in isolation, then synthesized into
reports. Only the orchestrator assigns work.
