 ...
 *                    movements (src/score.ts --> movement/*.md, read from
 ...
 *   repertoire       tool: the writing-style corpus as a tool (ported from
...
const DEBUG_LOG = path.join(homedir(), ".local", "share", "abstract", "logs", "hook.log");
  ...
  setup: async (ctx) => {
    ...
    await ctx.session.hook("context", (event) => {
      ...
      for (const stem of stems) {
        try {
          const text = fs.readFileSync(path.join(MOVEMENT_DIR, `${stem}.md`), "utf8").trim();
    ...
    await ctx.tool.transform((tools) => {
      tools.add({
        ...
        input: z.object({
          ...
          message: z.string().describe("Short pointer or question; not a document."),
        }),
        execute: async ({ target, message }, context) => {
          if (Buffer.byteLength(message, "utf8") > MAX_MESSAGE_BYTES)
            return { content: `cue exceeds ${MAX_MESSAGE_BYTES} bytes; put the content in a file and cue the path` };
          const api = client();
          const caller = await api.session.get({ sessionID: context.sessionID });
          const from = (caller.metadata?.role as string | undefined) ?? context.agent;
          if (from === target) return { content: `cannot cue yourself (${target})` };
          const listed = await api.session.list({ directory: caller.location.directory });
          const hit = listed.data.find((s) => s.metadata?.role === target && s.parentID === undefined);
          if (!hit)
            return { content: `no ${target} session in this project; it is created by \`abstract\` in the project directory` };
...
function repertoireTool() {
  return {
    ...
    input: z.object({
      ...
      ref: z.string().optional().describe("context: a hit ref like 10.1017:psy.2024.18#c046"),
      doi: z.string().optional().describe("restrict to one paper (outline target, or search filter)"),
      ...
      prefer: z.string().optional().describe("soft boost for a section bucket, results re-sorted"),
      year: z.number().optional().describe("filter on publication year"),
      topK: z.number().optional().describe("hits to return (default 6)"),
      maxPerPaper: z.number().optional().describe("max hits per paper (default 0 = unlimited; 1 = all-distinct papers)"),
      radius: z.number().optional().describe("context: chunks on each side of ref (default 2)"),
    }),
