...
export interface ConvertReport {
  ...
  bib: {
    ...
    manual: boolean;
    truncated: boolean;
    section_redefs: number;
  };
  ladder: string; // r0..r3 | ladder-exhausted | "" on early fail
  pandoc_err?: string;
  title: {
    how: "title" | "centerline" | "center-env" | "none";
    text: string | null;
  };
  abstract_env: boolean;
  paragraphs_demoted: number;
  refs_swept: boolean;
  fm_divs_dropped: number;
  fm_empty_math_dropped: number;
  headings_out: number;
  sections_src: number;
  chapters_src: number;
...
function pandocRun(
  ...
  outFile: string,
  toPlain = false,
): { ok: boolean; errHead?: string } {
  const r = spawnSync(
    PANDOC,
    [...(toPlain ? PANDOC_TO_PLAIN : PANDOC_TO_MD), "-o", outFile, inFile],
    {
      timeout: 60_000,
      maxBuffer: 1 << 24,
    },
...
function plain(fragment: string, dir: string): string {
  ...
  if (!pandocRun(inFile, outFile, true).ok)
    return fragment
      .replace(/\\[a-zA-Z]+\s*/g, "")
      .replace(/\s+/g, " ")
      .trim();
...
function extractTitle(
  source: string,
  dir: string,
): { title: string | null; how: ConvertReport["title"]["how"]; text: string } {
  ...
  if (m1) {
    ...
    if (t)
      return {
        title: t,
        how: "title",
        text: source.slice(0, m1.index) + source.slice(end),
      };
  }
  ...
  while ((mb = m1b.exec(source))) {
    ...
    if (t && t.length > 3)
      return {
        title: t,
        how: "title",
        text: source.slice(0, mb.index) + source.slice(end),
      };
  }
  ...
  while ((mm = m2.exec(source)) && mm.index < 8000) {
    ...
    if (!/\\(?:bf|bfseries|LARGE|Large|large|Huge|huge|sc)\b/.test(inner))
      continue;
    // single pass, space-preserving: two sequential strips once glued
    // \LARGE onto the following word (\LARGE\bf X -> \LARGEX -> dropped)
    const t = plain(
      inner.replace(
        /\\(?:bf|bfseries|it|itshape|rm|rmfamily|tt|ttfamily|sc|scshape|normalfont|Huge|huge|LARGE|Large|large|normalsize|small|footnotesize|scriptsize|tiny)\b\s*/g,
        " ",
      ),
      dir,
    );
    if (t && t.length > 3)
      return {
        title: t,
        how: "centerline",
        text: source.slice(0, mm.index) + source.slice(end),
      };
  }
    ...
    if (tb) {
      ...
      if (t && t.length > 3)
        return {
          title: t,
          how: "center-env",
          text: source.slice(0, mc.index) + source.slice(blockEnd),
        };
    }
    // font/bold brace group: {\bf T}, {\Large \bf T}, ...
    const fg =
      /\{\s*\\(?:bf|bfseries|Large|LARGE|large|Huge|huge|sc)[a-zA-Z]*(?:\s|\})/.exec(
        block,
      );
    if (fg) {
...
function extractAbstract(source: string): { abs: string | null; text: string } {
  ...
  return {
    abs: source.slice(i + "\\begin{abstract}".length, j),
    text: source.slice(0, i) + source.slice(j + "\\end{abstract}".length),
  };
}
...
export function sweepReferences(md: string): { text: string; swept: boolean } {
  ...
  return {
    text: lines.slice(0, lastRef).join("\n").replace(/\n+$/, "\n"),
    swept: true,
  };
}
...
// front-matter sweep (cluster test 2026-09-18): pandoc renders leftover
// \maketitle fragments as empty display math and ::: fenced divs that
// repeat the title above an author/date grid. Empty display math is
// noise anywhere; a fenced div is dropped when it appears in the first
...
export function frontMatterSweep(
  ...
  while (i < lines.length) {
    ...
    if (/^:::/.test(line) && out.length < 60) {
      // find div end and first content line
      let j = i + 1;
      let end = -1;
      ...
      let first = "";
      for (let k = i + 1; k < (end < 0 ? lines.length : end); k++) {
        if (lines[k].trim()) {
          first = lines[k].trim().replace(/^\*\*|\*\*$/g, "");
          break;
        }
      }
      if (titleNorm && norm(first) === titleNorm && end >= 0) {
        divs++;
        i = end + 1;
        continue;
      }
...
function srcMathCounts(src: string): { env: number; inline: number } {
  const env =
    (
      src.match(
        /\\begin\{(?:equation|align|eqnarray|gather|multline|displaymath|math|alignat|flalign|eqalign|split|IEEEeqnarray\*?|dmath\*?)\*?\}/g,
      ) || []
    ).length + (src.match(/\\\[/g) || []).length;
  let dollars = 0;
...
const refsAbsent = (md: string) =>
  !/^#{1,6}\s*(references|bibliography|literature)\b/im.test(md) &&
  !md.includes("\\begin{thebibliography}");

// ---------- convert ----------
export function convert(doiId: string): ConvertResult {
  const fail = (
    err: string,
    partial: Partial<ConvertReport> = {},
  ): ConvertResult => ({
    md: "",
    report: {
      doi_id: doiId,
  ...
  const base: Partial<ConvertReport> = {
    ...
    tex_members: ex.texMembers.length,
  };
  ...
  base.sections_src = (
    bib.text.match(/\\(?:section|subsection)\*?\s*[{\[]/g) || []
  ).length;
  ...
  if (a.abs !== null) {
    ...
    const absMd = pandocRun(absFile, absOut).ok
      ? fs.readFileSync(absOut, "utf8").trim()
      : "";
  ...
  const fm = frontMatterSweep(md, t.title);
  md = fm.text;
  base.fm_divs_dropped = fm.divs;
  base.fm_empty_math_dropped = fm.emptyMath;
  ...
  const report: ConvertReport = {
    ...
    headings_vs_sections: +(
      headingsOut /
      Math.max(1, (base.sections_src ?? 0) + (base.chapters_src ?? 0))
    ).toFixed(2),
    math_display_out: mo.display,
    math_inline_out: mo.inline,
    residual_bs: residual,
  ...
  if (residualPerKb > RESIDUAL_ESCALATE_PER_KB)
    reasons.push("residual-density");
...
if (import.meta.main) {
  ...
  if (args[0] === "--batch") {
    ...
    if (!idsFile || !outDir) {
      console.error(
        "usage: bun src/tex-convert.ts --batch <ids-file> --out <dir>",
      );
      process.exit(2);
    }
    ...
    for (const id of ids) {
      ...
      console.log(
        `${report.status.padEnd(4)} ${id} ladder=${report.ladder} h=${report.headings_out}/${(report.sections_src ?? 0) + (report.chapters_src ?? 0)} math=${report.math_display_out}d/${report.math_inline_out}i resid=${report.residual_per_kb}/KB${report.escalate ? ` ESCALATE[${report.escalate.reasons}]` : ""}`,
      );
    }
    console.log(
    ...
    console.error(
      "usage: bun src/tex-convert.ts <doi_id> | --batch <ids-file> --out <dir>",
    );
    process.exit(2);
  }
}
