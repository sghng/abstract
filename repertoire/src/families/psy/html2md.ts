...
function convert(
  ...
  // # title from the page h1 (contract: REQUIRED; cluster test 2026-09-18
  // gap). Fallback: <title> text before the " | " site suffixes.
  ...
  body.find("div.fig-ada, div.table-wrap-ada").each((_, el) => {
    ...
    const caption = $el.find("div.caption").text().replace(/\s+/g, " ").trim();
...
const main = async () => {
  ...
  console.log(
    `${figTotal} figures, ${tabTotal} tables; ${allSql.length} asset rows -> ${sqlPath}`,
  );
  if (APPLY) {
    ...
    console.log(
      (await proc.exited) === 0 ? "assets pushed to D1" : "D1 push FAILED",
    );
  }
};
