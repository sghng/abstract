#!/usr/bin/env bun
/**
 * JEM HTML backfill: full-text HTML for papers with NO "xml" row in the
 ...
 * doi_id|html (resume). Computed at start, right before capture begins.
 *
 * Capture routes (spike-jem, 2026-09-15; exact spike code paths):
 *   - year < 2005: page navigation (goto, challenge handling, second goto,
 *     response body capture) -- spike route for 1996/1998/2000/2002/2004
 *     (114-382KB, html-full). The request API 403s old content on this
 *     zone (spike: old-year xml 403), so nav-first avoids a dead attempt.
 *     Falls back to in-page fetch on nav failure.
 *   - year >= 2005: in-page fetch of /doi/full/<doi> (request API passes
 *     for modern content on this zone -- the xml/pdf fetcher rides it).
 *     Falls back to page-nav on 403/404/no-response/shell.
 *
 ...
 *
 * Conventions match fetch.ts in this dir, with the bjmsp fetcher's
 * timeout armor: block-page text -> clean stop stopped_reason=blocked-page;
 * challenge -> human handoff, 5 min waits, two unresolved strikes -> stop;
 * in-page fetch AbortController 120s with 140s evaluate race (a stalled
 * connection cannot wedge the run); 4-5s jittered pace after every
 * attempt; resume via manifest replay plus valid-on-disk manifest
 * adoption; 11.5h hard cap.
 *
 ...
 *
 * NOT AUTO-LAUNCHED: runs on the ORIGINAL .cache/cf-profile, which the
 * jem fetch session may still need; central launches this after that agent
 * closes out.
 *
 * Usage: bun repertoire/.cache/bulk/jem/backfill-html.ts
...
const PROFILE = argOf("--profile") ?? `${REPD}/.cache/cf-profile`;
const BASE = "https://onlinelibrary.wiley.com";
const WARMUP = "10.1111/jedm.12264"; // spike control on this zone
const JOURNAL = "jem";

// era routing: page-nav first below this year (spike-proven 1996-2004),
// in-page fetch first from it on (request API passes modern content here)
const INPAGE_FROM_YEAR = 2005;
...
async function waitClearance(label: string, waitMs: number): Promise<boolean> {
  ...
  console.log(`[human] ACTION NEEDED (${label}): solve the challenge in the JEM window`);
