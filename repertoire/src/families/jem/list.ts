#!/usr/bin/env bun
/**
 * Bulk JEM listing: Crossref journal 0022-0655 (Wiley), from 1996-01-01,
 * cursor-paginated, type=journal-article, front matter filtered.
 * Writes papers.jsonl rows {"doi","doi_id","title","year","journal":"jem","volume"}.
 *
 * Usage: bun repertoire/.cache/bulk/jem/list.ts
 */
import * as fs from "node:fs";

const BULK = new URL(".", import.meta.url).pathname;
// JEM registers under both the print ISSN (backfile) and the electronic
// ISSN (recent records); the union, deduped by DOI, is the full span.
const ISSNS = ["0022-0655", "1745-3984"];
...
for (const ISSN of ISSNS) {
  ...
  while (cursor) {
    const u =
      ...
      `&filter=from-pub-date:1996-01-01` +
      `&select=DOI,title,issued,volume,type`;
...
console.log(`non-10.1111 DOIs: ${odd}`);
