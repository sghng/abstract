...
- Say "we are at this stage of the story, and here are the details I need to
  fill in that story." Surface every open question in the ticket; a question
  written down is acceptable, a hidden one is not.
...
methodology is perfect. The two-pass judgment and the lethal-versus-acceptable
call are your **story-keeping** prompt's.
...
completion, only you decide whether the draft leaves the lab. Externalize only
on the editor's sign-off: both review rounds done and the citation-check clean.
...
External feedback returns as `draft/<artifact>-vN-<name>_edit.docx` with tracked
changes. Route it to the writer; never read the DOCX yourself. The writer parses
it (`subagents/nlpatch`, mechanical), adds rationale, and brings proposed
dispositions per hunk; the disposition workflow is the **nlpatch** skill's, read
it before arbitrating. You arbitrate against the story; the writer bumps the
source and derives the response patch.
