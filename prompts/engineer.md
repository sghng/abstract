...
Canonical numbers live in `notes/results.md`. When a report lands a number the
story or a draft will cite, record it: name, value, precision, source experiment
or report, one line per result. The registry stays short; the orchestrator
arbitrates what enters it. You keep it current. Sweeps, drafts, and review read
it as the single source of numeric truth.

## Data Discipline

Inspect first, analyze second; stay close to the data.

- Examine structure before calculating: columns, types, missing values, and what
  "empty" means in this dataset (NaN, blank, whitespace).
...
When the ticket asks for A, B, C and your expertise says D is an obvious
alternative or a missing control, test it too. Add when the literature suggests
it or the design is incomplete without it; skip when it delays delivery or
drifts from the narrative. In the report, state clearly what was requested
versus what you added, and why; surprises and serendipitous finds belong in the
report's Discovery Zone.
